...
export default function SharePointBrowser({
  ...
  async function loadDrives(site: SharePointSite) {
    ...
    try {
      ...
      setDrivesError(toErrorMessage(error, "Unable to load drives."));
    } finally {
      setIsLoadingDrives(false);
    }
  }

  async function loadFolderItems(crumbs: FolderCrumb[], driveOverride?: SharePointDrive) {
    const activeDrive = driveOverride || selectedDrive;
    ...
    try {
      if (crumbs.length === 0) {
        const response = await apiFetch<SharePointItem[]>(
          `/api/v1/sharepoint/drives/${encodeURIComponent(activeDrive.id)}/root/children`,
          { cache: "no-store" },
        );
        setFolderStack([]);
        setItems(response);
        ...
        const response = await apiFetch<SharePointItem[]>(
          `/api/v1/sharepoint/drives/${encodeURIComponent(activeDrive.id)}/items/${encodeURIComponent(active.id)}/children`,
          { cache: "no-store" },
        );
      ...
      setItemsError(toErrorMessage(error, "Unable to load folder items."));
    } finally {
  ...
  async function searchSites() {
    ...
    try {
      ...
      setSitesError(toErrorMessage(error, "Unable to search SharePoint sites."));
    } finally {
  ...
  async function openFilePreview(item: SharePointItem) {
    ...
    try {
      const preview = await apiFetch<SharePointPreviewResponse>(
        `/api/v1/sharepoint/items/${encodeURIComponent(item.id)}/preview?drive_id=${encodeURIComponent(selectedDrive.id)}`,
        { cache: "no-store" },
      );
      ...
      setPreviewKind("external");
      setPreviewError(toErrorMessage(error, "Unable to preview file."));
    } finally {
      ...
      <Card className="border-slate-200/70 shadow-sm">
        ...
        <CardContent className="space-y-4 pt-5">
          ...
          <div className="flex items-start gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
            ...
            <span>
              This uses Microsoft Graph API directly; no SharePoint iframe embedding is used.
            </span>
          </div>
        </CardContent>
      </Card>
                  ...
                  {isLoadingDrives ? (
                    ...
                    drives.map((drive) => (
                      <button
                        key={drive.id}
                        type="button"
                        onClick={() => void loadRootItems(drive)}
                        className={`w-full rounded-lg border px-3 py-2 text-left text-sm transition ${
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                    ...
                    {folderStack.map((crumb, index) => (
                      <button
                        key={`${crumb.id}-${index}`}
                        type="button"
                        onClick={() => void loadFolderItems(folderStack.slice(0, index + 1))}
