...
# MONEY: STRICT.
# This is deliberate: it prevents accidentally capturing unit qty "1" as money.
# If a document doesn't show "$", it is not safe to interpret it as currency here.
_MONEY_RX = re.compile(r"\(?\$\s*(?:[0-9]{1,3}(?:,[0-9]{3})+|[0-9]+)(?:\.[0-9]{1,2})?\)?")

# Claim block split anchor
...
def _extract_account_id(block: str) -> str | None:
    # Broad support: Account Number/No/ID/# + ACNT/ACCT + Patient Ctrl Nmbr
    explicit = re.search(
        r"(?im)(?:Patient\s*Ctrl\s*Nmbr|ACNT|ACCT|Account\s*(?:Number|ID|#|No\.?)?)\s*:\s*([^\n]+)",
        block or "",
    )
    if not explicit:
        return None
...
def _row_signals(text: str) -> dict[str, bool]:
    contains_adj = bool(re.search(r"\b(?:CO|PR|OA|PI)-\d+\b", text, flags=re.IGNORECASE))
...
def _find_adj_code(text: str) -> tuple[str | None, int | None]:
    m = re.search(r"\b(?:CO|PR|OA|PI)-\d+\b", text, flags=re.IGNORECASE)
    if not m:
        return None, None
...
def _pick_amount_near_keyword(tokens: list[tuple[int, Decimal]], text: str, keyword: str) -> Decimal | None:
    if not tokens:
        return None
    idx = (text or "").lower().find(keyword.lower())
    if idx < 0:
        return None
...
def _is_probably_junk_row(row: dict[str, Any], *, signals: dict[str, bool]) -> bool:
    ...
    if has_strong:
        return False
    # If we only had weak signals, mark as probably junk.
    return not (signals.get("has_line_ctrl") or signals.get("has_adj_code") or signals.get("has_hc_marker"))
...
def _fallback_table_rows(
    ...
    *,
    ...
    # Conservative fallback: only emit when we have date + proc + at least one $ amount.
    ...
    for idx, line in enumerate(lines):
        ...
        if not money_values:
            continue
        billed_amount = max([v for v in money_values if v > 0], default=None)
        paid_amount = money_values[0] if money_values else None
        rows.append(
            {
...
def _parse_era_table_layout(
    ...
    *,
    ...
    - Split logical rows primarily on "line ctrl" token near start.
    - Do NOT start new rows based on adjustment sub-lines.
    - Emit only rows that look like service lines: date + proc + billed $ amount.
    """
    rows: list[dict[str, Any]] = []
    stats = {
        "lines_in_region": 0,
        ...
        "dropped_reason_not_service_line": 0,
    }
    ...
    for i, line in enumerate(lines):
        if "line ctrl nmbr" in line.lower():
            header_idx = i
            break
    if header_idx < 0:
        return rows, stats
    ...
    def finalize_buffer(lines_buf: list[str]) -> None:
        ...
        positive = [val for _, val in amounts_with_pos if val > 0]
        ...
        # SERVICE LINE RULE: Only count/emit what can be a service line.
        # This is what fixes "13 instead of 3".
        if dos_from is None or proc_code is None or billed_amount is None:
            stats["dropped_rows_total"] += 1
            stats["dropped_reason_not_service_line"] += 1
            return
        ...
        row["row_is_probably_junk"] = _is_probably_junk_row(row, signals=_row_signals(joined))
    ...
    for line in region:
        ...
        # New row boundaries are primarily "line ctrl" near start.
...
def _parse_era_monospace_layout(
    ...
    *,
    ...
    """
    Monospace parser:
    - STRICT $ money tokens (prevents Decimal('1') from qty)
    - Emit only rows with date + proc + billed money
    """
    rows: list[dict[str, Any]] = []
    lines = _clean_lines(block)
    if not lines:
...
def parse_era_content(
    ...
    *,
    ...
    for block_idx, block in enumerate(blocks, start=1):
        ...
        counters["dropped_reason_not_service_line"] += table_stats.get("dropped_reason_not_service_line", 0)
    ...
    # Global suppression behavior (existing intent preserved)
...
def collect_content_diagnostics(content: str, *, mode: str) -> dict[str, int]:
    ...
    blocks = _split_billed_blocks(content) if mode == "billed" else _split_claim_blocks(content)
    counters["blocks_after_split"] = len(blocks)
    for block in blocks:
        account_id = _extract_account_id(block)
        if not account_id:
...
def parse_billed_content(content: str, billed_track: str | None = None) -> tuple[list[dict[str, Any]], dict[str, int]]:
    """
    Billed statement parser.
    billed_track is optional for backward compatibility; the deterministic parser does not depend on it.
    """
    _ = billed_track  # unused; kept for signature compatibility
    ...
    for block in _split_billed_blocks(content):
        ...
        for i, line in enumerate(lines):
            ...
            # Conservative: Only consider windows likely to include service lines.
            ...
            # Require a service-line signature to avoid junk rows.
            if parsed.get("dos_from") is None or not parsed.get("proc_code") or parsed.get("billed_amount") is None:
                continue
